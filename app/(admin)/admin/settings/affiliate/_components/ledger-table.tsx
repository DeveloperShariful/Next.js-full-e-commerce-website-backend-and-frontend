...
export default function LedgerTable({ data, totalEntries, currentPage, totalPages }: Props) {
  ...
  const getTypeStyles = (type: string) => {
    switch (type) {
      ...
      default:
        return { bg: "bg-gray-50", text: "text-gray-700", icon: ArrowRightLeft };
    }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4 bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
        <div className="flex gap-2 w-full sm:w-auto">
            ...
            <button className="flex items-center gap-2 px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50 bg-white shadow-sm">
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                  ...
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg border ${style.bg} ${style.text} bg-opacity-50`}>
                        ...
                        <div>
                          ...
                          <div className="text-[11px] text-gray-500 max-w-[250px] truncate leading-tight mt-0.5" title={item.description || ""}>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1 text-[10px] text-gray-500 bg-gray-50 p-2 rounded border border-gray-100 w-fit">
                        <div className="flex justify-between w-28">
                          <span>Before:</span> <span className="font-mono">{formatPrice(Number(item.balanceBefore))}</span>
                        </div>
                        <div className="flex justify-between w-28 font-bold text-gray-700">
                    ...
                    <td className="px-6 py-4 text-right">
                        ...
                        <div className="text-[10px] text-gray-400">{format(new Date(item.createdAt), "hh:mm a")}</div>
                    </td>
                  </tr>
                );
              })
        ...
        <div className="p-4 border-t bg-gray-50 text-xs text-gray-500 flex justify-between items-center">
            <span>Showing {data.length} of {totalEntries} transactions</span>
            <div className="flex gap-2">
                <button disabled={currentPage <= 1} className="px-3 py-1 bg-white border rounded hover:bg-gray-100 disabled:opacity-50">Previous</button>
                <button disabled={currentPage >= totalPages} className="px-3 py-1 bg-white border rounded hover:bg-gray-100 disabled:opacity-50">Next</button>
            </div>
        </div>
      </div>
    </div>
