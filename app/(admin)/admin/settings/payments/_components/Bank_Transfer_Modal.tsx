// app/admin/settings/payments/_components/Bank_Transfer_Modal.tsx
...
import { updateBankTransferSettings } from "@/app/actions/admin/settings/payments/offline-payment-method"
import { z } from "zod"
...
export const Bank_Transfer_Modal = ({ methodId, config, offlineConfig }: BankModalProps) => {
  ...
  const form = useForm<z.infer<typeof BankTransferSchema>>({
    ...
    defaultValues: {
      ...
      isEnabled: !!config.isEnabled, // ✅ Ensure boolean
      
      // ✅ FIX: Mapped DB data to Schema format (if DB has accountName, map to name)
      bankDetails: offlineConfig?.bankDetails && offlineConfig.bankDetails.length > 0 
        ? offlineConfig.bankDetails.map((bank: any) => ({
            name: bank.name || bank.accountName || "", // Handle both cases for safety
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Button 
                  ...
                  variant="outline" 
                  size="sm"
                  // ✅ FIX: Used 'name' instead of 'accountName'
              ...
              {fields.map((field, index) => (
                <div key={field.id} className="p-4 border rounded-lg space-y-3 bg-muted/20 relative">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <FormField
                      control={form.control}
                      // ✅ FIX: Field name matches Schema ('name')
