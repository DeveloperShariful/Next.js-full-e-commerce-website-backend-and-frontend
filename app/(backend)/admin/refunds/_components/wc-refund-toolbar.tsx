// File: app/admin/refunds/_components/wc-refund-toolbar.tsx

"use client";

import { useState } from "react";
import { bulkUpdateRefunds } from "@/app/actions/backend/refund/refund";
import { RefundQueryParams, GetRefundsResponse } from "../types";
import { ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight } from "lucide-react";
import { toast } from "sonner"; 

interface WcRefundToolbarProps {
  queryParams: RefundQueryParams;
  setQueryParams: (params: RefundQueryParams) => void;
  meta?: GetRefundsResponse["meta"];
  selectedIds: string[];
  onRefresh: () => void;
  setSelectedIds: (ids: string[]) => void;
}

export const WcRefundToolbar = ({
  queryParams,
  setQueryParams,
  meta,
  selectedIds,
  ...
  const handleBulkAction = async () => {
    ...
    const confirmMessage = bulkAction === "delete" 
        ? "Are you sure you want to delete selected records permanently?" 
        : `Are you sure you want to mark selected refunds as ${bulkAction.toUpperCase()}?`;

    if (!confirm(confirmMessage)) return;

    setIsUpdating(true);
    const res = await bulkUpdateRefunds(selectedIds, bulkAction);
  ...
  return (
    // মোবাইলে এলিমেন্টগুলো যাতে নিচে নিচে চলে আসে তার জন্য flex-col এবং gap বাড়ানো হয়েছে
    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3 py-2 text-[13px] text-[#2c3338] mb-2">
      ...
      <div className="flex items-center gap-1.5 w-full lg:w-auto">
        ...
        >
          ...
          <option value="approved">Approve refunds</option>
          <option value="rejected">Reject refunds</option>
