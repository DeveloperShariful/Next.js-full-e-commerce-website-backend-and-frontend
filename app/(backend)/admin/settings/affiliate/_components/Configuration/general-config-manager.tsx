// File: app/(backend)/admin/settings/affiliate/_components/Configuration/general-config-manager.tsx
...
export default function AffiliateGeneralConfigForm({ initialData, mlmInitialData }: Props) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...initialData,
      commissionType: initialData.commissionType || "PERCENTAGE", // Ensure default
    } as any,
  });
                ...
                <div className={cn(
                    ...
                    <div className="flex items-center gap-3">
                        <div className={cn("p-2 rounded-lg", form.watch("isActive") ? "bg-green-200 text-green-800" : "bg-gray-200 text-gray-500")}>
                            {form.watch("isActive") ? <CheckCircle className="w-5 h-5"/> : <Settings className="w-5 h-5" />}
                        </div>
                        <div>
                            <h4 className="text-sm font-bold text-gray-900">Program Status</h4>
                            <p className="text-xs text-gray-600">{form.watch("isActive") ? "Affiliate system is live and tracking." : "System is disabled. No commissions will be recorded."}</p>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="space-y-6">
                        ...
                        <section className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                            <SectionHeader title="Brand Identity" description="How the program appears to users." icon={Settings} />
                            <div className="grid gap-4">
                                <InputField name="programName" label="Program Name" placeholder="e.g. GoBike Partner Program" />
                                <InputField name="termsUrl" label="Terms & Conditions URL" placeholder={`${siteUrl}/terms`} type="url" />
                            </div>
                        </section>

                        {/* 2. Global Commission Logic (Crucial Update) */}
                        <section className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm transition-all hover:shadow-md">
                            <SectionHeader title="Default Commission" description="Base earnings for all partners (unless overridden)." icon={DollarSign} />
                            
                            <div className="space-y-4">
                                <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
                                    <label className="text-xs font-bold text-gray-700 uppercase mb-3 block">Global Rate</label>
                                ...
                                <div className="grid gap-3 pt-2">
                                    <ToggleField name="excludeShipping" label="Exclude Shipping Cost" description="Calculate commission on subtotal only." />
                                    <ToggleField name="excludeTax" label="Exclude Taxes" description="Deduct tax before calculating commission." />
                                    <ToggleField name="zeroValueReferrals" label="Track Zero Value Orders" description={`Record orders even if commission is ${currency}0.`} />
                                    <ToggleField name="autoApplyCoupon" label="Auto-Apply Coupon" description="Automatically apply affiliate's coupon on link click." />
                                </div>
                            </div>
                        </section>
                    </div>
                    ...
                    <div className="space-y-6">
                        ...
                        {/* 3. Link Structure */}
                        <section className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                            <SectionHeader title="Link Configuration" description="Referral link behavior and format." icon={LinkIcon} />
                            <div className="space-y-4">
                                <InputField name="referralParam" label="URL Query Parameter" placeholder="ref" prefix="?..." suffix="=123" />
                                
                                <div className="p-4 bg-gray-50 rounded-lg border border-gray-200 space-y-3">
                                    <ToggleField name="customSlugsEnabled" label="Allow Custom Slugs" description="Partners can create named links (e.g. /ref/summer-sale)" />
                                    {form.watch("customSlugsEnabled") && (
                                        <div className="grid grid-cols-2 gap-4 animate-in fade-in">
                                            <InputField name="slugLimit" label="Max Slugs per User" type="number" />
                                            <div className="flex items-end pb-1">
                                                <ToggleField name="autoCreateSlug" label="Auto-Generate" description="On signup" />
                                            </div>
                                        </div>
                                    )}
                                </div>
                        ...
                        {/* 4. Tracking Rules */}
                        <section className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                            <SectionHeader title="Tracking & Attribution" description="Cookie duration and fraud rules." icon={Shield} />
                            <div className="grid gap-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <InputField name="cookieDuration" label="Cookie Duration" type="number" suffix="Days" placeholder="30" />
                                    <InputField name="lifetimeDuration" label="Lifetime Link" type="number" suffix="Days" placeholder="∞" />
                                </div>
                                <ToggleField name="isLifetimeLinkOnPurchase" label="Lifetime Linking" description="Permanently lock customer to affiliate after first purchase." />
                                <ToggleField name="allowSelfReferral" label="Allow Self-Referral" description="Allow affiliates to earn commission on their own purchases." />
                            </div>
                        </section>

                        {/* 5. Payout Settings */}
                        <section className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                            <SectionHeader title="Payout Policy" description="Withdrawal rules and methods." icon={Clock} />
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <InputField name="minimumPayout" label="Min Withdrawal" type="number" prefix={currency} />
                                    <InputField name="holdingPeriod" label="Holding Period" type="number" suffix="Days" helperText="Wait time for refunds." />
                                </div>
                                ...
                                <div>
                                    ...
                                    {form.formState.errors.payoutMethods && <p className="text-red-500 text-[10px] mt-1">Select at least one method.</p>}
                                </div>
                                
                                <ToggleField name="autoApprovePayout" label="Auto-Approve Payouts" description="Skip manual review step (High Risk)." />
                            </div>
                        </section>
                    </div>
                </div>
