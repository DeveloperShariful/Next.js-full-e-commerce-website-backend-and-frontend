//File 11: app/admin/settings/payments/[identifier]/_components/Stripe/Stripe_Webhook_Config.tsx

"use client"

import { useState, useEffect } from "react"
import { PaymentGatewayUI } from "@/app/(backend)/admin/settings/payments/types-and-schemas"
import { refreshStripeWebhook, saveManualWebhook } from "@/app/actions/backend/settings/payments/stripe-actions"
...
export const Stripe_Webhook_Config = ({ method }: { method: PaymentGatewayUI }) => {
  ...
  const [manualSecret, setManualSecret] = useState(method.webhookSecret || "")
  ...
  const handleSync = async () => {
    if (!isConnected) {
      toast.error("Please connect Stripe API Keys first.");
      return;
    }
    
    setIsRefreshing(true)
    try {
      const res = await refreshStripeWebhook(method.id)
      if (res.success) {
  ...
  const handleManualSave = async () => {
    if (!manualSecret) {
      toast.error("Please enter a webhook secret first.");
      return;
    }

    setIsSaving(true)
    try {
      const res = await saveManualWebhook(method.id, manualSecret, method.webhookUrl || localUrl)
      if (res.success) {
        toast.success(res.message)
        router.refresh()
      } else toast.error(res.error)
    ...
    <div>
      ...
      {!isConnected ? (
         <div className="p-4 bg-red-50 border-b border-red-200 text-sm text-red-700">
           ⚠️ Please connect your Stripe API Keys in the "Connection" tab before setting up webhooks.
         </div>
      ) : (
        <div className={`p-4 border-b text-sm flex items-center gap-3 ${hasWebhook ? 'bg-green-50 text-green-800 border-green-200' : 'bg-yellow-50 text-yellow-800 border-yellow-200'}`}>
           ...
           <p>Webhook Status: <strong>{hasWebhook ? "Active" : "Not Configured"}</strong> ({isTestMode ? "Test" : "Live"} Environment)</p>
        </div>
      )}
      ...
      <div className="space-y-0">
        ...
        <FormRow label="Webhook Endpoint URL">
          <div className="flex items-center gap-2 p-2 bg-gray-100 border border-gray-300 rounded-sm text-gray-700 font-mono text-[13px] break-all">
              <Globe className="h-4 w-4 flex-shrink-0 text-[#635BFF]" />
              <span>{method.webhookUrl || localUrl}</span>
          </div>
          <p className="text-xs text-gray-500 mt-2">
            This is the URL where Stripe sends payment notifications. 
            You can either copy this and paste it in your Stripe Dashboard, or click the Auto Setup button below.
          </p>
        </FormRow>

        <FormRow label="Webhook Signing Secret">
          <div className="flex flex-col sm:flex-row gap-3">
             ...
             <Button 
               ...
               disabled={isSaving || !manualSecret || !isConnected} 
          ...
          <p className="text-xs text-gray-500 mt-2">If you added the webhook manually in Stripe, paste the secret (whsec_...) here and save.</p>
        </FormRow>
        ...
        <FormRow label="Automatic Setup" isLast>
          <Button 
            ...
            className="bg-[#635BFF] hover:bg-[#524BFF] text-white h-9 px-6 text-sm"
          >
          ...
          <p className="text-xs text-gray-500 mt-2">Clicking this will automatically create the webhook in your Stripe account and save the secret.</p>
        </FormRow>
