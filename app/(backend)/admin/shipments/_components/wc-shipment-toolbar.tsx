// File: app/admin/shipments/_components/wc-shipment-toolbar.tsx

"use client";

import { useState } from "react";
import { bulkUpdateShipments } from "@/app/actions/backend/shipment/shipment";
import { ShipmentQueryParams, GetShipmentsResponse } from "../types";
import { ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight } from "lucide-react";
import { toast } from "sonner";

interface WcShipmentToolbarProps {
  queryParams: ShipmentQueryParams;
  setQueryParams: (params: ShipmentQueryParams) => void;
  meta?: GetShipmentsResponse["meta"];
  selectedIds: string[];
  onRefresh: () => void;
  setSelectedIds: (ids: string[]) => void;
}

export const WcShipmentToolbar = ({
  queryParams,
  setQueryParams,
  meta,
  selectedIds,
  ...
  const handleBulkAction = async () => {
    ...
    const confirmMessage = bulkAction === "delete" 
        ? "Are you sure you want to delete selected shipments permanently?" 
        : "Are you sure you want to mark selected shipments as Delivered? This will also update the order status.";

    if (!confirm(confirmMessage)) return;

    setIsUpdating(true);
    const res = await bulkUpdateShipments(selectedIds, bulkAction);
  ...
  return (
    // মোবাইলের জন্য flex-col এবং gap বাড়ানো হয়েছে
    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3 py-2 text-[13px] text-[#2c3338] mb-2">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full lg:w-auto">
        ...
        {/* Pagination Controls - মোবাইলে যাতে না কাটে তাই flex-wrap */}
