...
export async function createCartHandoffAction(): Promise<CartHandoffResult> {
  try {
    ...
    if (generalConfig?.enableWalletEscapeHatch !== true) return { enabled: false };

    // ── Kill switch 2: secret must be configured ────────────────────────────
    if (!process.env.CART_HANDOFF_SECRET) return { enabled: false };
